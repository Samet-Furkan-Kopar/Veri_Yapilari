...
void IkiliSiralamaAgaci::agacKur(int* girdi) {
    ...
    yeni->sol = NULL;
    yeni->sag = NULL;
    bool kayýtEdildi = false;
    int karsilastir;
    if (kok == NULL) {//ilk düðüm ekleniyor
        kok = yeni;
    }
    while ((dolas != NULL) && (!kayýtEdildi))
    {
        karsilastir = yeni->veri - dolas->veri;
            if (karsilastir < 0)
            {
                if (dolas->sol != NULL)
                {
...
bool DFS(Dugum* kok, int aranacakVeri) {
    Dugum* dugum;
    stack<Dugum*> yigin;
    yigin.push(kok);
    while (!yigin.empty())
...
bool BFS(Dugum* kok, int aranacakVeri) {
    Dugum* dugum;
    queue<Dugum*> kuyruk;
    kuyruk.push(kok);
    while (!kuyruk.empty())
